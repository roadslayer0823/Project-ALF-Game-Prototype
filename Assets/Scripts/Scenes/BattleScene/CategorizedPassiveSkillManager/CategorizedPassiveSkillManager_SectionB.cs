...
public partial class CategorizedPassiveSkillManager : MonoBehaviour
{
    ...
    public static void RunCharacterExcitementEffect( ref BattleResultData battleResultData, ref List<string> resultLogList, GameCharacter gameCharacterOne, GameCharacter gameCharacterTwo )
    {
        ...
        //float pSL12_ShengShengBuXi_value = gameCharacterOne.生命積分 >= 250 && (gameCharacterOne.HasCategorizedPassiveSkill(PASSIVE_SKILL_ID_PSL12)) ? 0.2f : 0.0f;
        ...
        float gameCharacterOneSkillDamage = gameCharacterOne.GetAssignedSkill().GetCharacterSubskillData().GetSubskillData().AttackDamage * 0.3f;
        float gameCharacterTwoSkillDamage = gameCharacterTwo.GetAssignedSkill().GetCharacterSubskillData().GetSubskillData().AttackDamage * 0.3f;
    ...
    public static void CalculateLightAndHeavyRecipientHealthResult(ref BattleResultData battleResultData, ref List<string> resultLogList, GameCharacter assaulter, GameCharacter recipient)
    {
        ...
        float pSL3_MengLie_value = (recipient.HasCategorizedPassiveSkill(PASSIVE_SKILL_ID_PSL3) && assaulter.GetSelectedPassiveSkillCategoryType() == CategoryType.Life) ? 0.2f : 0.0f;
        //float assaulter_pSL12_ShengShengBuXi_value = (assaulter.生命積分 >= 250 && assaulter.GetSelectedPassiveSkillCategoryType() == CategoryType.Life && recipient.HasCategorizedPassiveSkill(PASSIVE_SKILL_ID_PSL12)) ? 0.2f : 0.0f;

        float pSL4_JianRen_value = (recipient.HasCategorizedPassiveSkill(PASSIVE_SKILL_ID_PSL4) && (recipient.GetSelectedPassiveSkillCategoryType() == CategoryType.Life)) ? 0.2f : 0.0f;
        //float recipient_pSL12_ShengShengBuXi_value = (recipient.生命積分 >= 250 && recipient.GetSelectedPassiveSkillCategoryType() == CategoryType.Life && recipient.HasCategorizedPassiveSkill(PASSIVE_SKILL_ID_PSL12)) ? 0.2f : 0.0f;
            ...
            (assaulter.GetSelectedPassiveSkillCategoryType() == CategoryType.None && recipient.GetSelectedPassiveSkillCategoryType() == CategoryType.Stress))
        {
            ...
            //recipientHealthPointDamage = assaulterSkillDamage * (1 + energyMarker_value) - ("重受擊方"已按下技能的減傷率);
        }
            ...
            assaulter.GetSelectedPassiveSkillCategoryType() == CategoryType.State || recipient.GetSelectedPassiveSkillCategoryType() == CategoryType.State)
        {
            ...
            //recipientHealthPointDamage = assaulterSkillDamage * (1 + pSL3_MengLie_value + assaulter_pSL12_ShengShengBuXi_value + energyMarker_value) * (1 - pSL4_JianRen_value - recipient_pSL12_ShengShengBuXi_value - pSE12_NiFeng_value) - ("重受擊方"已按下技能的減傷率);
        }

        //float finalHealthPointDamage = recipient.is重受擊方 ? recipientHealthPointDamage * breakStatus_value : recipientHealthPointDamage;
        //battleResultData.AddGameCharacterResultData_ActualHealthPointDamage(recipient, finalHealthPointDamage);
    }
}
