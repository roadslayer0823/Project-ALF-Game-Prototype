        ...
        if (_improviserCurrentSkill.GetCharacterSubskillData().GetSubskillData().Id == "S18_1")
        {
            ...
            if (_improviser_BattleResultData.currentStatePoint > _lead_BattleResultData.currentStatePoint)
            {
                ...
                if (lead.GetCurrentSkillRangeType() == RangeType.ranged)
                {
                    ...
                    // 進入“迎擊直擊方受擊方結算”頁面。
                    BattleLogicManagerV2.SettleRepulseResultForAssaulterAndRecipient( ref battleResultData, assaulter: improviser, recipient: lead );
        ...
        if (!gameCharacter.HasOneOfCharacterIdentityTypes( new CharacterIdentityType[] { CharacterIdentityType.SpeedLoser, CharacterIdentityType.SpeedStrengthLoser } ))
        {
            ...
            if (gameCharacter.GetCurrentSkillRangeType() == RangeType.melee)
            {
                ...
                if (opponent.GetCurrentSkillRangeType() == RangeType.ranged)
                {
                    ...
                    // "己方"[最大以太值]+[最大以太提升]
                    battleResultData.AddGameCharacterResultData_MaximumStatePointIncreaseForBonus( gameCharacter, _maxStatePointUp, out BattleResultData.BattleResultData_GameCharacter gameCharacterResultData );
