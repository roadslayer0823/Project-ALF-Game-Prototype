...
public partial class BattleAnimationManager: MonoBehaviour
{
    public ( AnimationParameterData animationParameterData, AnimationParameterData extraAnimationParameterData ) DetermineAnimationOfPlayerOneForPartB( GameCharacter playerOne, GameCharacter playerTwo )
    {
        ...
        {
            _getLastATLSkill = playerOne.GetLastAtlSkill().GetCharacterSubskillData().GetSubskillData().SkillId;
        }
        ...
        if (_playerOne_CurrentSkill != null)
        {
            ...
            _getCurrentSkill = _playerOne_CurrentSkill.GetCharacterSubskillData().GetSubskillData().SkillId;
        }
        ...
        if (playerOne.HasOneOfCharacterIdentityTypes( new CharacterIdentityType[] { CharacterIdentityType.Recipient, CharacterIdentityType.LightRecipient, CharacterIdentityType.HeavyRecipient } ))
        {
            ...
            else if (playerOne.HasCharacterIdentityType( CharacterIdentityType.Improviser ))
            {
                ...
                //己方是否"速度負方" /"速度強度負方" ?
                //YES
                else if (playerOne.HasOneOfCharacterIdentityTypes(new CharacterIdentityType[] { CharacterIdentityType.SpeedLoser, CharacterIdentityType.SpeedStrengthLoser }))
                {
                    //當前距離是否[中距離]或[遠距離] ?
                    if (_currentDistanceType is BattleDistanceManager.DistanceType.Normal or BattleDistanceManager.DistanceType.Far)
                    {
                    ...
                    //已按下技能是否與上1ATL 相同 & 上1ATL播放了"v1演出" & 有"v2演出" ?
                    else if (_playerOne_CharacterAnimationHandler.CheckIfSameAsLastATLCodeType("V1") && _playerOne_CharacterAnimationHandler.CheckIfAnyAnimationCodeTypeForCurrentSkillHasKeyword("CLSV2") &&
                ...
                //已按下技能是否與上1ATL 相同 & 上1ATL播放了"v1演出" & 有"v2演出" ?
                else if (_playerOne_CharacterAnimationHandler.CheckIfSameAsLastATLCodeType("V1") && _playerOne_CharacterAnimationHandler.CheckIfAnyAnimationCodeTypeForCurrentSkillHasKeyword("CLPV2") &&
                    playerOne.GetLastAtlSkill() == _playerOne_CurrentSkill)
                {
                    //己方是否"輕受擊方" ?
                    if (playerOne.HasCharacterIdentityType(CharacterIdentityType.LightRecipient))
    ...
    public ( AnimationParameterData animationParameterData, AnimationParameterData extraAnimationParameterData ) DetermineAnimationOfPlayerTwoForPartB( GameCharacter playerOne, GameCharacter playerTwo )
    {
        ...
        {
            _getLastATLSkill = _playerTwo_LastATLSkill.GetCharacterSubskillData().GetSubskillData().SkillId;
        }
        ...
        {
            _playerTwo_SubskillData = _playerTwo_CurrentSkill.GetCharacterSubskillData().GetSubskillData();
            _getCurrentSkill = _playerTwo_CurrentSkill.GetCharacterSubskillData().GetSubskillData().SkillId;
        }
        ...
        if (playerTwo.HasCharacterIdentityType( CharacterIdentityType.Deuce ))
        {
            ...
            if (playerTwo.HasCharacterIdentityType( CharacterIdentityType.Lead ))
            {
                ...
                // YES
                if (playerTwo.HasCharacterIdentityType( CharacterIdentityType.NormalDistanceMeleeDealer ) && _playerTwo_CharacterAnimationHandler.CheckIfAnyAnimationCodeTypeForCurrentSkillHasKeyword("VC"))
                {
        ...
        if (playerTwo.HasOneOfCharacterIdentityTypes( new CharacterIdentityType[] { CharacterIdentityType.Assaulter, CharacterIdentityType.LightAssaulter, CharacterIdentityType.HeavyAssaulter } ))
        {
            ...
            if (playerTwo.HasCharacterIdentityType( CharacterIdentityType.Lead ))
            {
                ...
                // 敵方是否"中距離近戰方"&已按下技能有"vc演出"?
                // YES
                else if (playerTwo.HasCharacterIdentityType( CharacterIdentityType.NormalDistanceMeleeDealer ) && _playerTwo_CharacterAnimationHandler.CheckIfAnyAnimationCodeTypeForCurrentSkillHasKeyword("VC"))
                {
                    _animationParameterData = new AnimationParameterData( false, true, CodeType.camB_type_BDVC, _playerTwo_SubskillId, _playerTwo_AnimationType );
                }
                //當前距離是否[中距離]或[遠距離] ?
        ...
        if (playerTwo.HasOneOfCharacterIdentityTypes( new CharacterIdentityType[] { CharacterIdentityType.Recipient, CharacterIdentityType.LightRecipient, CharacterIdentityType.HeavyRecipient } ))
        {
            ...
            else if (playerTwo.HasCharacterIdentityType( CharacterIdentityType.Improviser ))
            {
                ...
                else if (playerTwo.HasOneOfCharacterIdentityTypes( new CharacterIdentityType[] { CharacterIdentityType.SpeedLoser, CharacterIdentityType.SpeedStrengthLoser } ))
                {
                    ...
                    // 已按下技能是否與上1ATL相同&上1ATL播放了"v1演出"&有"v2演出"?
                    // YES
                    else if (_playerTwo_CharacterAnimationHandler.CheckIfSameAsLastATLCodeType("V1") && _playerTwo_CharacterAnimationHandler.CheckIfAnyAnimationCodeTypeForCurrentSkillHasKeyword("CLSV2") &&
                        _getLastATLSkill == _getCurrentSkill)
                    {
                        // 敵方是否"輕受擊方"?
                        // YES
                ...
                // 已按下技能是否與上1ATL相同&上1ATL播放了"v1演出"&有"v2演出"?
                // YES
                else if (_playerTwo_CharacterAnimationHandler.CheckIfSameAsLastATLCodeType("V1") && _playerTwo_CharacterAnimationHandler.CheckIfAnyAnimationCodeTypeForCurrentSkillHasKeyword("CLPV2") &&
                    _getLastATLSkill == _getCurrentSkill)
                {
                    // 敵方是否"輕受擊方"?
                    // YES
