...
{
    // 頁面：判定防禦成敗及結算
    private static void DetermineResultForDefense( ref BattleResultData battleResultData, GameCharacter lead, GameCharacter improviser )
    {
        // 進行"後手方"[當前以太值]的結算,
        // 參考：
        // "後手方"的已按下的技能的[以太消耗],"近距離遠程"的標記(以太消耗的基礎值+20%)
        ...
        if (_hasImproviserDefendedSuccessfully)
        {
            ...
            // "受擊方"的[能量殘響]
            // 受擊方當前以太值結算
            CategorizedPassiveSkillManager.RunRecipientCurrentStatePointCalculation( ref battleResultData, recipient: improviser, assaulter: lead );
            ...
            if (!lead.HasCharacterIdentityType( CharacterIdentityType.SpeedWinner ))
            {
                ...
                // 3.化勁 / 9.行雲流水
                // 受擊方負荷值結算
                CategorizedPassiveSkillManager.RunRecipientCurrentStressValueCalculation( ref battleResultData, assaulter: lead, recipient: improviser );
            }
            ...
            if (_improviser_BattleResultData.currentStressValue >= 100.0f)
            {
                ...
                battleResultData.AddGameCharacterResultData_StressBreakStatus( improviser, 1, out _ );
            ...
            {
                // 調用“判定輕重受擊方”頁面。
                CategorizedPassiveSkillManager.RunDeterminingLightOrHeavyRecipient( ref battleResultData, assaulter: lead, recipient: improviser );
            }
        ...
        else
        {
            ...
            // "抵抗成功方"的[能量殘響]
            // 抵抗成功方防禦當前以太值結算
            CategorizedPassiveSkillManager.SuccessfulResisterDefenseCurrentStatePointCalculation( ref battleResultData, successfulResister: improviser, deuce: lead );
            ...
            // 3.化勁 / 9.行雲流水
            // 抵抗成功方防禦負荷值結算
            CategorizedPassiveSkillManager.RunSuccessfulResisterDefenseStressValueCalculation( ref battleResultData, successfulResister: improviser, deuce: lead );

            BattleResultData.BattleResultData_GameCharacter _successfulResister_BattleResultData = battleResultData.GetGameCharacterResultData( improviser );
        ...
        // "玩家1"進入“負荷積分等級結算”頁面。
        CategorizedPassiveSkillManager.StressScoreAndLevelCalculation( ref battleResultData, lead );
        ...
        // "玩家2"進入“負荷積分等級結算”頁面。
        CategorizedPassiveSkillManager.StressScoreAndLevelCalculation( ref battleResultData, improviser );
